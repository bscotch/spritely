...
export class Spritely {
  ...
  async crop(extraPadding = 1) {
    ...
    await Promise.all(
      this.paths.map(async (path) => {
        ...
        const subimageBoundingBox = Spritely.getForegroundBounds(
          img,
          extraPadding,
        );
        if (this.allowSubimageSizeMismatch) {
          ...
          await img
            .crop(Spritely.cropObjectFromBoundingBox(subimageBoundingBox))
            .save(path);
          return;
        }
        ...
        boundingBox.right = Math.max(
          boundingBox.right,
          subimageBoundingBox.right,
        );
        boundingBox.bottom = Math.max(
          boundingBox.bottom,
          subimageBoundingBox.bottom,
        );
      }),
    );
    if (!this.allowSubimageSizeMismatch) {
      ...
      await Promise.all(
        this.paths.map((path) => Spritely.cropImage(path, cropObject)),
      );
    }
    return this;
  }
  ...
  static async bleed(imagePath: string, options?: SpriteEdgeCorrectionOptions) {
    ...
    const foreground = Spritely.getForegroundMask(
      img,
  ...
  private static cropObjectFromBoundingBox(boundingBox: {
    left: number;
    top: number;
    right: number;
    bottom: number;
   ...
   */
  private static getForegroundMask(
    image: ImageExt,
